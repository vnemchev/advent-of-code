function processor(input: string) {
    ...
    passports.forEach((pass: string): number | undefined => {
        const fields =
            pass.match(pattern)!.map((f: string) => {
                const [k, v] = f.split(':');
                return k;
            }) || undefined;
        ...
        if (missingFields.length === 1 && missingFields[0] === optionalField) {
            return validCount++;
        }
...
console.log(
    processor(`eyr:2029 byr:1931 hcl:z cid:128

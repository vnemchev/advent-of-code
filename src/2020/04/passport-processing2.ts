function processor2(input: string) {
    const passports = input.split('\n\n');
    const pattern = /[a-z]{3}:\w*#*\w*/gm;
    const optionalField = 'cid';

    let validPassCount = 0;

    passports.forEach((pass: string) => {
...
function validator([key, value]: [param: string, param: string]): boolean {
    const requiredFields = {
        byr: (birthYear: string): boolean => {
            const bYear = Number(birthYear);
            return birthYear.length === 4 && bYear >= 1920 && bYear <= 2002;
        },
        iyr: (issueYear: string): boolean => {
            const iYear = Number(issueYear);
            return issueYear.length === 4 && iYear >= 2010 && iYear <= 2020;
        },
        eyr: (expYear: string): boolean => {
            const eYear = Number(expYear);
            return expYear.length === 4 && eYear >= 2020 && eYear <= 2030;
        },
        hgt: (height: string): boolean => {
            const structureValid = /[0-9]+[in|cm]+/.test(height);

            if (!structureValid) return false;

            const metric = height.substring(height.length - 2);
            const numHeight = Number(height.substring(0, height.length - 2));

            if (metric === 'in') return numHeight >= 59 && numHeight < 76;
            else return numHeight >= 150 && numHeight < 193;
        },
        hcl: (hairColor: string): boolean => {
            return /#[0-9a-f]{6}/.test(hairColor);
        },
        ecl: (eyeColor: string): boolean => {
            return /amb|blu|brn|gry|grn|hzl|oth/.test(eyeColor);
        },
        pid: (passID: string): boolean => {
            return /[0-9]{9}/.test(passID);
        },
    };

    return requiredFields[key as keyof typeof requiredFields](value);
}

// 0 valid passes - correct!
console.log(
    processor2(`eyr:1972 cid:100
...
// 4 valid passes - correct!
console.log(
    processor2(`pid:087499704 hgt:74in ecl:grn iyr:2012 eyr:2030 byr:1980
hcl:#623a2f
...
// 107 valid passes - incorrect!
console.log(
    processor2(`eyr:2029 byr:1931 hcl:z cid:128

function main(input: string): number {
    ...
    let boxesWithDouble: string[] = [];
    let boxesWithTriple: string[] = [];

    for (const box of boxes) {
        const checkedChars: string[] = [];
        for (const char of box) {
            if (checkedChars.includes(char)) continue;
            checkedChars.push(char);

            const pattern = new RegExp(char, 'g');
            const matches = box.match(pattern);
            const matchesL = matches?.length;
...
console.log(
    main(`mgdsdwqulhrinkatfjoyzxcbvq
